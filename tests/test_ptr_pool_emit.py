"""Regression sul codegen del pool puntatori ibrido (statico + dinamico).

Modello: slot < heap_base = celle nominate `__mn_mem*` (dispatch `if slot==k`,
bancato se heap_base > ~998); slot >= heap_base = heap VM dinamico (procedure
`_dyn` con ops POOLPUSH/POOLADD/POOLGET su `vm->mn_pool`, cresce on-demand).
Verifica strutturale (veloce): eseguire un programma con > 998 celle nominate è
troppo lento per la CI (dispatch O(N) nell'interprete), ma il forward+heap è già
coperto da test_pool_runtime_loop e dai gcc-compat malloc.
"""
...
class TestPtrPoolEmit(unittest.TestCase):
    def test_monolithic_plus_dyn(self) -> None:
        ...
        for op in ("poolpush(slot,", "pooladd(slot,", "poolget(slot,"):
