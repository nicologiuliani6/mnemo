 ...
 * des.c - Single-file DES implementation (ECB single block)
...
static uint64_t permute(uint64_t in,const int *tbl,int n,int bits){

    uint64_t out=0;

    for(int i=0;i<n;i++){ out<<=1; out|=(in>>(bits-tbl[i]))&1ULL; }
...
static uint32_t feistel(uint32_t r,uint64_t k){
    ...
    uint64_t e=permute(r,E,48,32)^k;
...
static void key_schedule(uint64_t key,uint64_t sub[16]){

    uint64_t k=permute(key,PC1,56,64);
...
uint64_t des_encrypt(uint64_t block,uint64_t key){
    ...
    uint64_t sub[16]; key_schedule(key,sub);

    block=permute(block,IP,64,64);

    uint32_t L=block>>32,R=(uint32_t)block;
...
int main(void){
    ...
    uint64_t ct =des_encrypt(pt,key);

    uint64_t dec=des_decrypt(ct,key);
